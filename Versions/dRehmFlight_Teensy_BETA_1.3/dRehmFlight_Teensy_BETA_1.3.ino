...
float GyroErrorX = 3.36;
float GyroErrorY = 1.31;
float GyroErrorZ = 10.41 + 0.278;
...
float Kp_pitch_angle = 0.2; // 0.2;   //Pitch P-gain - angle mode
float Ki_pitch_angle = 0.5; // 0.3;   //Pitch I-gain - angle mode
float Kd_pitch_angle = 0.1; // 0.05;  //Pitch D-gain - angle mode (has no effect on controlANGLE2)
...
void loop()
{
  ...
  if (checkMechSetupCommand())
    isMechSetupMode = !isMechSetupMode;
  if (checkCalibCommand())
    calculate_IMU_error();
...
class PulseTrap
{
  ...
  bool old_execute = false;
  int PTstatus = 0;
  float retval = 0;

  public:
  float Run(bool execute, float fadeTime_delay, float fadeTime_up, float fadeTime_duration, float fadeTime_down, int loopFreq)
  {

    elapsed_time += 1 / (float)loopFreq;
    ...
    case 0:
      elapsed_time = 0;
      retval = 0;
      if (execute && !old_execute)
        PTstatus = 10;
      break;

    case 10:
      retval = 0;
      if (elapsed_time > fadeTime_delay)
        PTstatus = 20;
      break;

    case 20:
      retval += 1 / (fadeTime_up * loopFreq);
      if (elapsed_time > fadeTime_delay + fadeTime_up)
        PTstatus = 30;
      break;

    case 30:
      if (elapsed_time > fadeTime_delay + fadeTime_up + fadeTime_duration)
        PTstatus = 40;
      retval = 1;
      break;
    ...
    case 40:
      ...
      if (elapsed_time > fadeTime_delay + fadeTime_up + fadeTime_duration + fadeTime_down)
        PTstatus = 99;
      break;
...
void controlMixer()
{
  ...
  thro_boost = 0;
  thro_boost += 0.2 * TB_hov2ff.Run(fader >0 , 1, 0.5, 1, 0.5, 2000);
  ...
  float ff_pid_boost, p=0.7;
  ff_pid_boost = 1+ p/2 - p*constrain(thro_des,0.2,0.8); 
...
void getCommands()
{
  ...
  if (sbus.read(&sbusChannels[0], &sbusFailSafe, &sbusLostFrame))
  {
    ...
    if ((sbusLostFrame || sbusFailSafe) && countFailsafe < FS_MAXCOUNT)
      countFailsafe++;
    else
    {
      countFailsafe = 0;
...
bool checkCalibCommand()
{
  ...
  if (channel_1_pwm < 1050 && channel_4_pwm > 1950 && channel_2_pwm < 1050 && channel_3_pwm < 1050 && channel_5_pwm > 1250 && !isFailsafe)
  {
...
bool checkMechSetupCommand()
{
  ...
  if (channel_1_pwm > 1950 && channel_4_pwm > 1950 && channel_2_pwm < 1050 && channel_3_pwm > 1950 && channel_5_pwm > 1250 && !isFailsafe)
  {
